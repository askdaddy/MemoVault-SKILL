# Classification scheme

How notes are organized in the vault: by domain (area) and by heat (maturity).
This is the spec the helper and the agent follow.

## 1. Directory layout

```
$AGENT_MEMO_VAULT/
  brain/
    <domain>/
      <Title>.md
      _<domain>-MOC.md        # domain index (generated by `moc`)
  daily/
    YYYY-MM-DD.md             # legacy / human Obsidian habit
  templates/                  # optional Obsidian Templates (not agent write source)
  .memovault/
    ledger.log                # protocol events; not searchable
```

Notes live under `brain/<domain>/`. The domain is also recorded in frontmatter so
queries do not depend on the path.

## 2. Frontmatter contract

```yaml
---
title: Human Readable Title
domain: engineering
kind: atom                 # optional; see section 3
tags: [rust, async]
heat: seedling
status: active             # optional: active | superseded; omit = active
supersedes: []             # optional; titles this note replaces
aliases: [Short Name]
sources: []                # optional; fill when distilled from raw/daily
created: 2026-07-13
updated: 2026-07-13
---
```

Field rules:
- `title` - human readable, unique in the vault. Matches the filename stem.
- `domain` - lowercase, single token or `kebab-case`. Matches the folder name.
- `kind` - optional. One of `raw`, `atom`, `scenario`, `persona`, `skill`.
  Omit to keep ordinary domain-note behavior.
- `tags` - YAML list. Nested tags use `parent/child` (e.g. `lang/rust`).
- `heat` - exactly one of `seedling`, `growing`, `evergreen`.
- `status` - optional. `active` (default when omitted) or `superseded`.
  Default `search`/`recall` exclude superseded notes unless
  `--include-superseded`. Use `supersede <old> <new>` to mark replacement.
- `supersedes` - optional YAML list of titles this note replaces.
- `aliases` - YAML list of alternate names; enables `[[Short Name]]` resolution.
- `sources` - optional YAML list of note titles or vault-relative paths that
  evidence this note. Required when distilling from raw/daily into a structured
  note. Also add `[[wikilinks]]` in the body so the graph stays connected.
- `created` / `updated` - ISO date. `updated` is bumped on edits.

## 3. Memory kinds (optional)

Optional `kind` maps layered memory onto the existing vault without a second
folder tree. Inspired by L0-L3 + Skill assets; recall prefers higher structure.

| `kind` | Role | Typical home | Default heat on `new` |
|---|---|---|---|
| `raw` | Fleeting evidence (L0) | `brain/inbox/` (preferred); `daily/` is legacy | seedling |
| `atom` | Fact, decision, constraint (L1) | `brain/<domain>/` | seedling |
| `scenario` | Project or scene block (L2) | `brain/<domain>/` + MOCs | seedling |
| `persona` | Stable preference / hard constraint (L3) | rare domain notes | seedling |
| `skill` | Reusable SOP (Trigger / Steps / Verify) | `brain/skills/` | growing |

Rules:
- Omit `kind` when unsure; treat as an ordinary domain note.
- Distill raw/daily into `atom` or `scenario`; set `sources` and body wikilinks.
- Never delete raw evidence without explicit user consent.
- `persona` stays rare. Do not start as `evergreen` unless the user confirms;
  otherwise seedling, then suggest `promote`.
- Skill notes use Trigger / Steps / Verify / Related (protocol body recipe).
  Vault `templates/skill.md` is optional for humans in Obsidian; it is not the
  agent write source.

## 4. Domains

A domain is a stable area of knowledge. Examples: `engineering`, `product`,
`research`, `ops`, `reading`, `people`, `skills`, `projects/<name>`.

Rules:
- Keep the set small and stable. Prefer folding into an existing domain over
  creating a new one.
- Domain folder name == `domain` frontmatter value.
- Every domain has a MOC index note (`moc <domain>` regenerates it).
- Skill SOPs live under `brain/skills/` (`domain: skills`).

## 5. Heat tiers

| Tier | Meaning | Typical signals |
|---|---|---|
| `seedling` | Just captured, raw, unverified. | New note, few or no links. |
| `growing` | Being developed, linked into the graph. | >=2 incoming backlinks, edited recently, cited by another domain. |
| `evergreen` | Mature, reliable, frequently referenced. | >=5 incoming backlinks or explicitly curated by the user. |

Promotion rules:
- The agent SUGGESTS promotion; the user confirms. `suggest` looks at 30-day
  ledger `cite`/`read`/`feedback` plus incoming backlinks (not heat auto-bump).
- `health` may also hint `distill_inbox` / `high_orphan_pct` / new 0.7.2 hints;
  still no silent rewrite.
- Promotion bumps `heat` and `updated`.
- Demotion is manual only (the agent never demotes on its own).

## 6. MOC (Map of Content)

`moc <domain>` writes `brain/<domain>/_<domain>-MOC.md`:

```markdown
---
title: <Domain> MOC
domain: <domain>
tags: [moc]
heat: evergreen
aliases: [<Domain> index]
---
# <Domain>

## evergreen
- [[Note A]]

## growing
- [[Note B]]

## seedling
- [[Note C]]
```

The MOC is regenerated on demand; it is not hand edited.

## 7. Daily notes (legacy)

`daily/YYYY-MM-DD.md` is **legacy / optional for humans** browsing in Obsidian.
The agent main path does **not** use daily as L0 evidence.

Agent L0: `kind: raw` notes, typically under `brain/inbox/`. Distill into
`atom` or `scenario` with `sources` and body `[[raw-title]]` (or use the
`distill` helper). Never delete raw evidence without explicit user consent.

Existing `daily/` files remain readable. `search`/`recall` do not scan `daily/`
by default. `health` may report `legacy_daily_count` as a migration hint.

Canonical date wikilink `[[YYYY-MM-DD]]` still resolves when a daily file exists
(compatibility with older notes).

## 8. Naming conventions

- Note filenames: human readable Title Case, matching the `title` field. Avoid
  slashes and colons (filesystem and Obsidian constraints).
- Daily filenames: `YYYY-MM-DD.md`.
- Slugs in tags/domains: lowercase `kebab-case`.

## 9. Deduplication

Before creating a note, the agent must run `dedupe "<title>"` and prefer
`append` on hits. Also check `search` / `backlinks` / `aliases` when the title
is not an exact match. If two notes clearly overlap, suggest a manual merge:
read both, append consolidated content into one, then `supersede` the other
(or leave a stub `[[<Title>]]` pointer). There is no `merge` subcommand.
